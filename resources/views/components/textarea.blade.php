@props(['name' => null, 'required' => null, 'label' => null, 'information' => null])


<div class="relative z-0 w-full mb-6 group">
    @if ($errors->has($name))
            <div class="relative z-0">
                <textarea id="{{ $name }}" rows="1" name="{{ $name }}" value="{{ old($name) }}" aria-describedby="standard_error_help"
                    class="block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-red-600 appearance-none focus:outline-none focus:ring-0 focus:border-red-600 peer"
